// Atlas Codex Worker - Simple Firecrawl-based extraction
...
const firecrawl = process.env.FIRECRAWL_API_KEY ? new Firecrawl({ 
...
// Simple extraction using Firecrawl
async function performExtraction(jobId, params) {
  try {
    ...
    // Use Firecrawl for extraction (using new API)
    const scrapeResult = await firecrawl.scrape(params.url, {
    ...
    if (!scrapeResult || !scrapeResult.success) {
      throw new Error(scrapeResult?.error || 'Firecrawl extraction failed');
    }
    ...
    const result = {
      success: true,
      data: {
        title: scrapeResult.data?.metadata?.title || 'Extracted Content',
        content: scrapeResult.data?.markdown || scrapeResult.data?.html || 'No content extracted',
        url: params.url,
        metadata: {
...
// Fallback extraction without Firecrawl
async function basicExtraction(jobId, params) {
  try {
    console.log(`Using basic extraction for job ${jobId}:`, params.url);
    
    await updateJobStatus(jobId, 'processing');
    
    // Simple fetch-based extraction
    ...
    await updateJobStatus(jobId, 'completed', result);
    console.log(`Job ${jobId} completed with basic extraction`);
    return result;
    
  } catch (error) {
    console.error(`Basic extraction failed for job ${jobId}:`, error);
    await updateJobStatus(jobId, 'failed', null, error.message);
    throw error;
  }
}
  ...
  for (const record of event.Records) {
    try {
      ...
      switch (type) {
        ...
        case 'scrape':
          try {
            // Try Firecrawl first, fallback to basic extraction
            result = await performExtraction(jobId, params);
          } catch (error) {
            console.log(`Firecrawl failed for ${jobId}, trying basic extraction:`, error.message);
            result = await basicExtraction(jobId, params);
          }
          break;
